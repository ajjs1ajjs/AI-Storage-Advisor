...
class AIProvider(ABC):
    ...
    def get_recommendation_system_prompt(self) -> str:
        return (
            ...
            "CRITICAL: You MUST NOT generate delete:// links for system files, libraries (.dll), application binaries (.exe), database files, or active application data/models (like Ollama libraries, Chrome models, pagefile.sys, game files) even if they take up a lot of space, as deleting them can break applications. Only generate delete:// links for temporary files (Temp) and log files (Log) that are 100% safe to delete without breaking any software."
        )
